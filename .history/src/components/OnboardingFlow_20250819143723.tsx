...
const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ onComplete }) => {
  ...
  // Add visual indicator for arrow navigation
  React.useEffect(() => {
    // Show navigation hint on first render
    const showHint = setTimeout(() => {
      // You can add a toast notification here if desired
      console.log('💡 Tip: Use ← → arrow keys to navigate between steps');
    }, 1000);

    return () => clearTimeout(showHint);
  }, []);
      ...
      <div className="max-w-6xl w-full">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center space-x-2 bg-blue-50 border border-blue-200 rounded-full px-4 py-2 text-sm text-blue-700">
            <span>⌨️</span>
            <span>Use <kbd className="px-2 py-1 bg-white rounded border border-blue-300 text-xs">←</kbd> <kbd className="px-2 py-1 bg-white rounded border border-blue-300 text-xs">→</kbd> arrow keys to navigate</span>
          </div>
        </div>
