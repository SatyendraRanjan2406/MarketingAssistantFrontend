  ...
  const handleGoogleConnect = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Find the popup window by name
        const checkPopup = setInterval(() => {
          popupWindow = window.open('', 'google-oauth');
          if (popupWindow) {
            popupWindow.close(); // Close the reference window
            popupWindow = null;
          }
        }, 1000);
        ...
        const checkClosed = setInterval(() => {
          if (popupWindow && popupWindow.closed) {
            ...
            if (!storedState) {
              // No state means OAuth was completed or failed
              // Check if we need to reset the connecting state
              if (isConnectingGoogle) {
                setIsConnectingGoogle(false);
                // Don't show error if connection was successful
                if (!connectedAccounts.google) {
                  setConnectionError('OAuth window was closed. Please try again if the connection was not completed.');
                }
              }
            }
          }
