  ...
  async initiateGoogleOAuth(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (response.data.success && response.data.authorization_url) {
        ...
        console.log('Popup window object:', authWindow);
        ...
        if (authWindow) {
          ...
          try {
            ...
            console.error('Popup window blocked or inaccessible:', popupError);
            authWindow.close();
            return { success: false, error: 'Popup window was blocked. Please allow popups for this site and try again.' };
          }
          
          return { success: true };
        } else {
          console.error('Failed to create popup window - window.open returned null');
